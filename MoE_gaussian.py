...
class ModelMerger(nn.Module):
    def __init__(self, all_models, num_layers=12):
        ...
        # 存储每个模型每层的μ和σ，形状为 [7, num_layers, hidden_size]
        self.mu_params = nn.Parameter(torch.zeros(num_layers, 7, 384), requires_grad=False)
        self.sigma_params = nn.Parameter(torch.ones(num_layers, 7, 384), requires_grad=False)
    ...
    def forward(self, x, eva=False, num_datasets=-1):
        merged_output = 0
        ...
        # 按照层级顺序对每层进行合并计算
        for i, mu, sigma in zip(range(self.num_layers), self.mu_params, self.sigma_params):
            merged_res = 0
            cls = x[:, 0]  # 取 x 的第一个元素，即 cls_token
            #cls = x[:, 1:, :].mean(dim=1)  # [batch_size, hidden_size]
            
            #scores 初始化
            if i == 0:
                result = torch.ones(x.shape[0], 7).to(device)
                self.scores = F.softmax(result, dim=1).to(device)
            ...
            if num_datasets >= 0:
                for j, model in enumerate(self.all_models):
                    if j == num_datasets:
                        ...
                        mu[num_datasets] = cls.mean(dim=0)  # shape: [hidden_size]
                        sigma[num_datasets] = cls.std(dim=0)  # shape: [hidden_size]
                        result = torch.ones(x.shape[0], 7).to(device)
                        self.scores = F.softmax(result, dim=1).to(device)
            else:
                if i!=0:
                    # 1. 计算 (cls - mu) / sigma，扩展维度以匹配 [B, 7, hidden_size]
                    cls_expanded = cls.unsqueeze(1)  # [B, 1, hidden_size]
                    mu_expanded = mu.unsqueeze(0)  # [1, 7, hidden_size]
                    sigma_expanded = sigma.unsqueeze(0)  # [1, 7, hidden_size]
    
                    # 计算每个样本在每个高斯分布下的对数概率
                    log_prob = -0.5 * torch.sum(((cls_expanded - mu_expanded) ** 2) / (sigma_expanded ** 2) + torch.log(2 * torch.pi * sigma_expanded ** 2), dim=-1)  # batch_size x 7
                    # 将对数概率转换为概率值
                    # tmp_scores = F.softmax(log_prob, dim=1)
                    # self.scores = (tmp_scores+self.scores)/2  # 结果大小为 [B, 7]
                    self.scores = F.softmax(log_prob, dim=1)

                for j, model in enumerate(self.all_models):
                    lora_a = model.blocks[i].attn.LoRA_a
                    lora_b = model.blocks[i].attn.LoRA_b
                    score = self.scores[0][j].to(x.device)
                    # 合并 LoRA_a 和 LoRA_b 权重
                    if merged_lora_a_weight is None:
                        merged_lora_a_weight = lora_a.weight * score
                        merged_lora_b_weight = lora_b.weight * score
                    else:
                        merged_lora_a_weight += lora_a.weight * score
                        merged_lora_b_weight += lora_b.weight * score
                    total_weight += score
...
class ModelMergerTome(nn.Module):
    def __init__(self, all_models, num_layers=12):
        ...
        # 存储每个模型每层的μ和σ，形状为 [7, num_layers, hidden_size]
        self.mu_params = nn.Parameter(torch.zeros(num_layers, 7, 768), requires_grad=False)
        self.sigma_params = nn.Parameter(torch.ones(num_layers, 7, 768), requires_grad=False)
    ...
    def forward(self, x, eva=False, num_datasets=-1):
        ...
        for i, mu, sigma in zip(range(self.num_layers), self.mu_params, self.sigma_params):
            ...
            if num_datasets >= 0:
                for j, model in enumerate(self.all_models):
                    if j == num_datasets:
                        ...
                        if r > 0:
                            ...
                            x, model.blocks[i]._tome_info["size"], weight_1, weight_2 = merge_wavg(merge, x_processed, model.blocks[i]._tome_info["size"], 
                                                                    pyra_weight=None,
                ...
                x_processed = model.blocks[i].attn.proj(x_processed)
                x_processed = model.blocks[i].attn.proj_drop(x_processed)
                x = x + model.blocks[i]._drop_path1(x_processed)
                ...
                if r > 0:
                    ...
                    x, model.blocks[i]._tome_info["size"], weight_1, weight_2 = merge_wavg(merge, x, model.blocks[i]._tome_info["size"], 
                                                            pyra_weight=None,
                                                            #pyra_weight=merged_pyra,
                                                            is_training=model.blocks[i].training)
...
model_paths_lora_high = [
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/cifar100_lr-0.001_wd-0.0001/best_checkpoint.pth',
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/caltech101_lr-0.001_wd-0.0001/best_checkpoint.pth',
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/dtd_lr-0.001_wd-0.0001/best_checkpoint.pth',
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/oxford_flowers102_lr-0.001_wd-0.0001/best_checkpoint.pth',
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/oxford_pet_lr-0.001_wd-0.0001/best_checkpoint.pth',
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/sun397_lr-0.001_wd-0.0001/best_checkpoint.pth',
                '/home/xiongyizhe/DualEfficient/PYRA/outputs/240920_ViT-S_prompt_lora_4_plain_lora/svhn_lr-0.001_wd-0.0001/best_checkpoint.pth',
                ]
model_paths_tome = [
...
all_models = []
for nb_classes, path in zip(classes_per_task, model_paths_lora_high):
    model = models.__dict__['vit_small_patch16_224_in21k'](   
                                                    img_size=224,
                                                    drop_rate=args.drop,
                                                    drop_path_rate=args.drop_path,
                                                    prompt_tuning_dim=0,LoRA_dim=4,adapter_dim=0,prefix_dim=0,
...
for epoch in range(1):
    ...
    for dataset_name, dataset_path in zip(exam_datasets, dataset_paths):
        ...
        for samples, targets in metric_logger.log_every(data_loader_train, print_freq, header):
            ...
            output = merger(samples, num_datasets=i)
            break
        i += 1

metric_logger.synchronize_between_processes()
print("Averaged stats:", metric_logger)
print(merger.mu_params[0][0])
