...
#                  pyra_r=None, num_experts=5, gating_temperature=1.0):
#         super().__init__()
...
#         # Adapter setting
#         self.adapter = Adapter(embed_dims=dim, reduction_dims=adapter_dim, drop_rate_adapter=drop_rate_adapter)
        
#         # Pyra modules for MoE
#         self.num_experts = num_experts

#         #gating_temperature for softmax
#         self.gating_temperature = gating_temperature
        
#         # Initialize multiple pyra modules (experts)
#         self.pyra_a_list = nn.ParameterList()
#         self.pyra_b_list = nn.ParameterList()
#         self.pyra_norm_list = nn.ModuleList()
#         for _ in range(num_experts):
#             if pyra_r is not None and pyra_r > 0:
#                 pyra_a = init_pyra_scale(pyra_r, 1, init="sqrt")
#                 pyra_b = init_pyra_scale(1, dim, init="zero")
#                 pyra_norm = nn.LayerNorm(dim)

#                 self.pyra_a_list.append(pyra_a)
#                 self.pyra_b_list.append(pyra_b)
#                 self.pyra_norm_list.append(pyra_norm)
#             else:
#                 break
#                 # 存储每个模型每层的μ和σ，形状为 [7, num_layers, hidden_size]
#         self.mu_params = nn.Parameter(torch.zeros(num_experts, dim), requires_grad=False)
#         self.sigma_params = nn.Parameter(torch.ones(num_experts, dim), requires_grad=False)
        
#         # Gating FFN (Mixture of Experts routing network)
#         self.gating = nn.Sequential(
#             nn.Linear(dim, 64),  # Input layer
#             nn.ReLU(),            # Activation function
#             nn.Linear(64, num_experts)  # Output layer: num_experts outputs
#         )
        
#     def gating_function(self, x):
#         #应当尝试用key gating
#         """Gating function that assigns each input to one or more experts."""
#         # Apply a simple linear transformation and softmax to get the gate probabilities
#         gate_logits = self.gating(x)
#         # gate_logits_sum = gate_logits.sum(dim=1)  # shape [B, num_experts]
#         gate_probs = F.softmax(gate_logits / self.gating_temperature, dim=-1)  # Softmax 输出 [B, num_experts]
...
class VisionTransformer(nn.Module):
    ...
    def freeze_stages(self):
        ...
        for name,param in self.named_parameters():
            if 'adapter' not in name and 'prompt' not in name and 'LoRA' not in name and 'prefix' not in name and 'head' not in name and 'pyra' not in name and 'gating' not in name:
    ...
    def forward_features(self, x, task_labels=None, preinference=False, epoch=0):
        B = x.shape[0]
    ...
    def forward(self, x, task_labels=None, preinference=False, epoch=0):
        x = self.forward_features(x, task_labels, preinference, epoch)
        if self.head_dist is not None:
            x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
