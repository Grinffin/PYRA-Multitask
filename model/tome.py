...
def task_specific_loss(gate_probs, labels, num_experts):
    if labels==None:
        return 0
    # 假设 labels 是一个形状为 (batch_size,) 的tensor，其中每个标签是一个整数，表示任务的索引
    # 生成一个one-hot编码的标签矩阵
    labels_onehot = F.one_hot(labels, num_classes=num_experts).float()  # (batch_size, num_experts)
    
    # gate_probs 是一个 (batch_size, num_experts) 的tensor，表示每个专家的选择概率
    # 计算任务标签和gate_probs之间的交叉熵损失
    loss = -torch.sum(labels_onehot * torch.log(gate_probs + 1e-8), dim=1)  # 避免log(0)
...
class ToMeBlock(Block):
    ...
    def forward(self, x: torch.Tensor, task_labels=None, preinference=False, epoch=0) -> torch.Tensor:
        B = x.shape[0]
        ...
        #         merged_weight_1 = 0
        #         expert_outputs = 0
        #         for i, (pyra_a, pyra_b, pyra_norm) in enumerate(zip(self.pyra_a_list, self.pyra_b_list, self.pyra_norm_list)):
        #             if i==task_labels:
        #                 pyra_expert = (pyra_a, pyra_b, pyra_norm)
        ...
        #                     )
        #                 expert_output, self._tome_info["size"], weight1, weight2 = merge_wavg(merge, x, self._tome_info["size"], 
        #                                                         pyra_weight=pyra_expert,
        #                                                         is_training=self.training)
        #                 merged_weight_1 = merged_weight_1 + weight1
        #                 expert_outputs = expert_outputs + expert_output
                    
        #                 self.mu_params[task_labels] = merged_weight_1.mean(dim=0)  # shape: [hidden_size]
        #                 self.sigma_params[task_labels] = merged_weight_1.std(dim=0)  # shape: [hidden_size]
        #     x = expert_outputs + self.adapter(self.drop_path(self.mlp(self.norm2(expert_outputs))))
        #     return x
        # 推理阶段
        # if r > 0:
        #     self.scores = self.gating_function(x[:,0])
        #     expert_outputs = []
        #     for i, (pyra_a, pyra_b, pyra_norm) in enumerate(zip(self.pyra_a_list, self.pyra_b_list, self.pyra_norm_list)):
        #         pyra_expert = (pyra_a, pyra_b, pyra_norm)
        #         merge, _ = bipartite_soft_matching(
        ...
        #                 merge, x, self._tome_info["source"]
        #             )
        #         expert_output, tmp_size, weight1, weight2 = merge_wavg(merge, x, self._tome_info["size"], 
        #                                                 pyra_weight=pyra_expert,
        #                                                 is_training=self.training)
        #         expert_outputs.append(expert_output)
        #     self._tome_info["size"] = tmp_size

        #     y = 0
        #     for k in range(self.num_experts):
        #         y += expert_outputs[k] * self.scores[:, k].view(B, 1, 1)
        #     x = y + self.adapter(self.drop_path(self.mlp(self.norm2(y))))
        ...
        if r > 0:
            ...
            x, self._tome_info["size"], weight1, weight2 = merge_wavg(merge, x, self._tome_info["size"], 
