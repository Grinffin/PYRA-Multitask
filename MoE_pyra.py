    ...
    def forward(self, x, eva=False, num_datasets=-1):
        ...
        for i, mu, sigma in zip(range(self.num_layers), self.mu_params, self.sigma_params):
            ...
            for j, model in enumerate(self.all_models):
                ...
                if i != 0:
                    model_scores = self.scores[:, j]  # 对应模型的分数为 [B]
                else:
                    result = torch.ones(B, 7).to(device)
                    self.uni_scores = F.softmax(result, dim=1).to(device)
                    model_scores = self.uni_scores[:, j]  # 对应模型的分数为 [B]
            ...
            if num_datasets < 0:
                ...
                # 将对数概率转换为概率值
                self.scores = F.softmax(log_prob, dim=1)  # 结果大小为 [B, 7]
                #print(self.scores)
