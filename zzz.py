...
class ModelMergerTome(nn.Module):
    def __init__(self, all_models, num_layers=12):
        ...
        #     nn.Parameter(torch.zeros(7, size), requires_grad=False)
        #     for size in [40, 34, 30, 24, 18, 14, 10, 8, 4, 4, 3, 3]
        # ])
        # self.sigma_params = nn.ParameterList([
        #     nn.Parameter(torch.ones(7, size), requires_grad=False)
        #     for size in [40, 34, 30, 24, 18, 14, 10, 8, 4, 4, 3, 3]
        # ])
    ...
    def forward(self, x, eva=False, num_datasets=-1):
        ...
        for i, mu, sigma in zip(range(self.num_layers), self.mu_params, self.sigma_params):
            ...
            if i == 0:
                ...
                self.uni_scores = F.softmax(result, dim=1).to(device)
                    ...
                    if j == num_datasets:
                        ...
                        if r > 0:
                            ...
                            x, model.blocks[i]._tome_info["size"], weight_1, weight_2 = merge_wavg(merge, x_processed, model.blocks[i]._tome_info["size"], 
                                                                    ...
                                                                    #pyra_weight=self.all_models[6].blocks[i].pyra,
                                                                    is_training=model.blocks[i].training)
                    
                        mu[num_datasets] = weight_1.mean(dim=0)  # shape: [hidden_size]
                        sigma[num_datasets] = weight_1.std(dim=0)  # shape: [hidden_size]
                        result = torch.ones(x.shape[0], 7).to(device)
                        self.scores = F.softmax(result, dim=1).to(device)
                ...
                if i>=0:
                    ...
                    for j, model in enumerate(self.all_models):
                        ...
                        if r > 0:
                            ...
                            x_processed, model.blocks[i]._tome_info["size"], weight_1, weight_2 = merge_wavg(merge, x, model.blocks[i]._tome_info["size"], 
                                                                    #pyra_weight=model.blocks[i].pyra,
                                                                    pyra_weight=self.all_models[6].blocks[i].pyra,
                                                                    is_training=model.blocks[i].training)

                        merged_weight_1 = merged_weight_1# + weight_1 * self.uni_scores[:, j].view(B, 1, 1)
                        merged_res.append(x_processed)
                        break
                else:
                    for j, model in enumerate(self.all_models):
                        attn_size = model.blocks[i]._tome_info["size"] if model.blocks[i]._tome_info["prop_attn"] else None
                        ...
                        if r > 0:
                            ...
                            x_processed, model.blocks[i]._tome_info["size"], weight_1, weight_2 = merge_wavg(merge, x_processed, model.blocks[i]._tome_info["size"], 
                                                                    pyra_weight=model.blocks[i].pyra,
            ...
            if num_datasets < 0:
                ...
                merged_weight_1_expanded = merged_weight_1  # [B, 1, hidden_size]
                mu_expanded = mu.unsqueeze(0)  # [1, 7, hidden_size]
                sigma_expanded = sigma.unsqueeze(0)  # [1, 7, hidden_size]

                # 计算每个样本在每个高斯分布下的对数概率
                log_prob = -0.5 * torch.sum(((merged_weight_1_expanded - mu_expanded) ** 2) / (sigma_expanded ** 2) + torch.log(2 * torch.pi * sigma_expanded ** 2), dim=-1)  # batch_size x 7
                # 最后对 r 维度求和，得到最终结果的形状：[B, 7]
                ...
                y = 0
                for k in range(7):
                    y += merged_res[k]# * self.scores[:, k].view(B, 1, 1)
                    break

                    # 3. 使用 softmax 正则化对数似然得到 scores
                    # scores = F.softmax(log_likelihood, dim=1)  # 结果大小为 [B, 7]
                x = y + model.blocks[i].adapter(model.blocks[i].drop_path(model.blocks[i].mlp(model.blocks[i].norm2(y))))
