...
def get_args_parser():
    ...
    parser.add_argument('--pyra', action='store_true', default=True)
...
def main(args):
    ...
    if args.eval:
        ...
        #classes_per_task = [400]
        test_stats = evaluate(data_loader_val, model, device, classes_per_task, args.offset, amp=args.amp)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.val_interval == 0 or epoch == args.epochs-1:
            classes_per_task = [100, 102, 47, 102, 37, 397, 10]
            #classes_per_task = [400]
            test_stats = evaluate(data_loader_val, model, device, classes_per_task, amp=args.amp)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
